...
const withMCQ = () => (tree) => {
  ...
  for (let i = tree.children.length - 1; i > -1; i--) {
    ...
    if (
      tree.children[i].type === "jsx" &&
      tree.children[i].value.match(hidRegex)
    ) {
      ...
      const prompt = toMarkdown({
        type: "root",
        children: tree.children.slice(i - j + 1, i - 1),
      });
      const solutions = solution.match(/[a-zA-z,]+/)[0];
      console.log(
        solutions
          .split(",")
          .map((solution) =>
            answers.findIndex(
              (answer) => answer.match(/[a-zA-Z]+/)[0] === solution
            )
          )
      );
      const correct_idxs = solutions
        .split(",")
        .map((solution) =>
          answers.findIndex(
            (answer) => answer.match(/[a-zA-Z]+/)[0] === solution
      ...
      tree.children[i].value = `<MCQ {...${JSON.stringify({
        id,
        solution,
        answers,
        prompt,
      ...
      tree.children = [
        ...tree.children.slice(0, i - j),
        ...tree.children.slice(i, tree.children.length),
      ];
    }
  }
};
